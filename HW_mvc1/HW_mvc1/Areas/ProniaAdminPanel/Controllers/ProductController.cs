...
namespace HW_mvc1.Areas.ProniaAdminPanel.Controllers
{
	...
	public class ProductController : Controller
	{
		...
		public async Task<IActionResult> Update(int? id, UpdateProductVM productVM)
		{
			...
			Product existed = await _context.Products.Include(x => x.ProductTags).Include(x => x.ProductSizes).Include(x => x.ProductColors).FirstOrDefaultAsync(x => x.Id == id && x.IsDeleted == false);
